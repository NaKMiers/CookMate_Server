...
import { handleQuery } from '@/lib/common'
      ...
      >
        ...
        {currentPage != 1 && (
          <Link
            ...
            className="trans-200 hover:bg-secondary rounded-lg border-2 border-white bg-white px-2 py-[6px] hover:text-white"
        ...
        <div className="no-scrollbar flex gap-2 overflow-x-scroll">
          {Array.from({ length: pageAmount }).map((_, index) => (
            <Link
              ...
              className={`trans-200 text-dark hover:bg-secondary rounded-lg border-2 border-white px-4 py-[6px] hover:text-white ${
                currentPage === index + 1
        ...
        {currentPage != pageAmount && (
          <Link
            href={getPageLink(
              currentPage >= pageAmount ? pageAmount : currentPage + 1
            )}
            className="trans-200 hover:bg-secondary rounded-lg border-2 border-white bg-white px-2 py-[6px] hover:text-white"
            title={`👉 Page ${currentPage >= pageAmount ? pageAmount : currentPage + 1}`}
          >
            Next
          </Link>
        )}
